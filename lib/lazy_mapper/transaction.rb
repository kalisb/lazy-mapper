module LazyMapper
  class Transaction
    ...
    #
    # Create a new LazyMapper::Transaction
    #
    # @see LazyMapper::Transaction#link
    #
    ...
    #   LazyMapper::Adapters::AbstractAdapter subclasses will be added as
    #     adapters as is.
    #   Arrays will have their elements added.
    #   LazyMapper::Repositories will have their @adapters added.
    #   LazyMapper::Resource subclasses will have all the repositories of all
    #     their properties added.
    #   LazyMapper::Resource instances will have all repositories of all their
    #     properties added.
    ...
    def link(*things, &block)
      ...
      things.each do |thing|
        if thing.is_a?(Array)
          link(*thing)
        elsif thing.is_a?(LazyMapper::Adapters::AbstractAdapter)
          @adapters[thing] = :none
        elsif thing.is_a?(LazyMapper::Repository)
          link(thing.adapter)
        elsif thing.is_a?(Class) && thing.ancestors.include?(LazyMapper::Resource)
          link(*thing.repositories)
        elsif thing.is_a?(LazyMapper::Resource)
          link(thing.class)
        else
    ...
    def method_missing(meth, *args, &block)
      if args.size == 1 && args.first.is_a?(LazyMapper::Adapters::AbstractAdapter)
        ...
        @adapters.each do |adapter, state|
          on_fail.each do |fail_handler|
            begin
              ...
              LazyMapper.logger.fatal("#{self}#each_adapter(#{method.inspect}, #{on_fail.inspect}) failed with #{e.inspect}: #{e.backtrace.join("\n")} - and when sending #{fail_handler} to #{adapter} we failed again with #{e2.inspect}: #{e2.backtrace.join("\n")}")
            end
          end
        end
        raise e
    ...
    def do_adapter(adapter, what, prerequisite)
      ...
      LazyMapper.logger.debug("#{adapter.name}: #{what}")
      @transaction_primitives[adapter].send(what)
      @adapters[adapter] = what
    end

    def log_fatal_transaction_breakage(adapter)
      LazyMapper.logger.fatal("#{self} experienced a totally broken transaction execution. Presenting member #{adapter.inspect}.")
    end
