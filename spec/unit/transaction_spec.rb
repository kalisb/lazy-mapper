...
describe LazyMapper::Transaction do
  ...
  before :each do
    ...
    @adapter.stub(:is_a?).with(Array).and_return(false)
    @adapter.stub(:is_a?).with(LazyMapper::Adapters::AbstractAdapter).and_return(true)
    @adapter.stub(:transaction_primitive).and_return(@transaction_primitive)
    @repository.stub(:is_a?).with(Array).and_return(false)
    @repository.stub(:is_a?).with(LazyMapper::Adapters::AbstractAdapter).and_return(false)
    @repository.stub(:is_a?).with(LazyMapper::Repository).and_return(true)
    @repository.stub(:adapter).and_return(@repository_adapter)
    @repository_adapter.stub(:is_a?).with(Array).and_return(false)
    @repository_adapter.stub(:is_a?).with(LazyMapper::Adapters::AbstractAdapter).and_return(true)
    @repository_adapter.stub(:transaction_primitive).and_return(@repository_transaction_primitive)
    @transaction_primitive.stub(:respond_to?).with(:close).and_return(true)
    @transaction_primitive.stub(:respond_to?).with(:begin).and_return(true)
    @transaction_primitive.stub(:respond_to?).with(:prepare).and_return(true)
    @transaction_primitive.stub(:respond_to?).with(:rollback).and_return(true)
    @transaction_primitive.stub(:respond_to?).with(:rollback_prepared).and_return(true)
    @transaction_primitive.stub(:respond_to?).with(:commit).and_return(true)
    @repository_transaction_primitive.stub(:respond_to?).with(:close).and_return(true)
    @repository_transaction_primitive.stub(:respond_to?).with(:begin).and_return(true)
    @repository_transaction_primitive.stub(:respond_to?).with(:prepare).and_return(true)
    @repository_transaction_primitive.stub(:respond_to?).with(:rollback).and_return(true)
    @repository_transaction_primitive.stub(:respond_to?).with(:rollback_prepared).and_return(true)
    @repository_transaction_primitive.stub(:respond_to?).with(:commit).and_return(true)
  end
  ...
  it "should initialize with no transaction_primitives" do
    LazyMapper::Transaction.new.transaction_primitives.empty?.should == true
  end
  it "should initialize with state :none" do
    LazyMapper::Transaction.new.state.should == :none
  end
  it "should initialize the adapters given on creation" do
    LazyMapper::Transaction.new(Smurf).adapters.should == {Smurf.repository.adapter => :none}
  end
  ...
  describe "#begin" do
    ...
    it "should try to connect each adapter (or log fatal error), then begin each adapter (or rollback and close)" do
      @transaction.stub(:each_adapter).with(:connect_adapter, [:log_fatal_transaction_breakage])
      @transaction.stub(:each_adapter).with(:begin_adapter, [:rollback_and_close_adapter_if_begin, :close_adapter_if_none])
      @transaction.begin
    end
  ...
  describe "#rollback" do
    ...
    it "should try to rollback each adapter (or rollback and close), then then close (or log fatal error)" do
      @transaction.stub(:each_adapter).with(:connect_adapter, [:log_fatal_transaction_breakage])
      @transaction.stub(:each_adapter).with(:begin_adapter, [:rollback_and_close_adapter_if_begin, :close_adapter_if_none])
      @transaction.stub(:each_adapter).with(:rollback_adapter_if_begin, [:rollback_and_close_adapter_if_begin, :close_adapter_if_none])
      @transaction.stub(:each_adapter).with(:close_adapter_if_open, [:log_fatal_transaction_breakage])
      @transaction.stub(:each_adapter).with(:rollback_prepared_adapter_if_prepare, [:rollback_prepared_and_close_adapter_if_begin, :close_adapter_if_none])
      @transaction.begin
  ...
  describe "#commit" do
    describe "without a block" do
      ...
      it "should try to prepare each adapter (or rollback and close), then commit each adapter (or log fatal error), then close (or log fatal error)" do
        @transaction.stub(:each_adapter).with(:connect_adapter, [:log_fatal_transaction_breakage])
        @transaction.stub(:each_adapter).with(:begin_adapter, [:rollback_and_close_adapter_if_begin, :close_adapter_if_none])
        @transaction.stub(:each_adapter).with(:prepare_adapter, [:rollback_and_close_adapter_if_begin, :rollback_prepared_and_close_adapter_if_prepare])
        @transaction.stub(:each_adapter).with(:commit_adapter, [:log_fatal_transaction_breakage])
        @transaction.stub(:each_adapter).with(:close_adapter, [:log_fatal_transaction_breakage])
        @transaction.begin
        @transaction.commit
      end
  ...
  describe "#within" do
    ...
    it "should push itself on the per thread transaction context of each adapter and then pop itself out again" do
      @repository_transaction_primitive.stub(:begin)
      @transaction_primitive.stub(:begin)
      @repository_adapter.stub(:push_transaction).with(@transaction)
      @adapter.stub(:push_transaction).with(@transaction)
      @repository_adapter.stub(:pop_transaction)
      @adapter.stub(:pop_transaction)
      @transaction.begin
  ...
  describe "#method_missing" do
    before :each do
      @transaction = LazyMapper::Transaction.new(@adapter, @repository)
      @adapter.stub(:is_a?).with(Regexp).and_return(false)
    end
    it "should delegate calls to [a method we have]_if_[state](adapter) to [a method we have](adapter) if state of adapter is [state]" do
      @transaction.stub(:state_for).with(@adapter).and_return(:begin)
      @transaction.stub(:connect_adapter).with(@adapter)
    ...
    it "should not delegate calls to [a method we have]_if_[state](adapter) to [a method we have](adapter) if state of adapter is not [state]" do
      @transaction.stub(:state_for).with(@adapter).and_return(:commit)
    ...
    it "should delegate calls to [a method we have]_unless_[state](adapter) to [a method we have](adapter) if state of adapter is not [state]" do
      @transaction.stub(:state_for).with(@adapter).and_return(:none)
      @transaction.stub(:connect_adapter).with(@adapter)
  ...
  describe "#each_adapter" do
    before :each do
      @transaction = LazyMapper::Transaction.new(@adapter, @repository)
      @adapter.stub(:is_a?).with(Regexp).and_return(false)
      @repository_adapter.stub(:is_a?).with(Regexp).and_return(false)
    end
    it "should send the first argument to itself once for each adapter" do
      @transaction.stub(:plupp).with(@adapter)
      @transaction.stub(:plupp).with(@repository_adapter)
    ...
    it "should stop sending if any call raises an exception, then send each element of the second argument to itself with each adapter as argument" do
      ...
      @transaction.stub(:plupp).with(@repository_adapter).and_throw(Exception.new("test error - dont mind me"))
      @transaction.should_not_receive(:plupp).with(@adapter)
      @transaction.stub(:plur).with(@adapter)
      @transaction.stub(:plur).with(@repository_adapter)
    ...
    it "should send each element of the second argument to itself with each adapter as argument even if exceptions occur in the process" do
      ...
      @transaction.stub(:plupp).with(@repository_adapter).and_throw(Exception.new("test error - dont mind me"))
      @transaction.should_not_receive(:plupp).with(@adapter)
      @transaction.stub(:plur).with(@adapter).and_throw(Exception.new("another test error"))
      @transaction.stub(:plur).with(@repository_adapter).and_throw(Exception.new("yet another error"))
      lambda do @transaction.instance_eval do each_adapter(:plupp, [:plur]) end end.should raise_error(Exception, /test error - dont mind me/)
    end
  end
  ...
  describe "#do_adapter" do
    ...
    it "should delegate to the adapter if the connection exists and we have the right state" do
      ...
      @transaction.begin
      a1 = @adapter
      @transaction_primitive.stub(:ping)
  ...
  describe "#connect_adapter" do
    before :each do
      ...
      @other_adapter.stub(:is_a?).with(Array).and_return(false)
      @other_adapter.stub(:is_a?).with(LazyMapper::Adapters::AbstractAdapter).and_return(true)
      @transaction = LazyMapper::Transaction.new(@other_adapter)
    end
    it "should be able to connect an adapter" do
      a1 = @other_adapter
      @other_adapter.stub(:transaction_primitive).and_return(@transaction_primitive)
  ...
  describe "the transaction operation methods" do
    ...
    it "should only allow adapters in state :prepare to rollback_prepared" do
      a1 = @other_adapter
      @transaction.stub(:do_adapter).with(@other_adapter, :rollback_prepared, :prepare)
      @transaction.instance_eval do rollback_prepared_adapter(a1) end
    end
    it "should do delegate properly for rollback_and_close" do
      a1 = @other_adapter
      @transaction.stub(:rollback_adapter).with(@other_adapter)
      @transaction.stub(:close_adapter).with(@other_adapter)
    ...
    it "should do delegate properly for rollback_prepared_and_close" do
      a1 = @other_adapter
      @transaction.stub(:rollback_prepared_adapter).with(@other_adapter)
      @transaction.stub(:close_adapter).with(@other_adapter)
